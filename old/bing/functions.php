...
$html_template= '<a href="{img_source}" title="{img_title}"><img src="{img_url}" alt="{img_title}" title="{img_title}"/></a><br>image thumb: {img_thumb}<br>url agc: http://amiikanaz.ga/search{url_agc}<br>image title: {img_title}<br>image info: {img_info}<br>image source: {img_source}';
return $html_template;
}

function bing_images($opl,$jumlah=1,$nomer=0){
$url = 'http://www.bing.com/images/search?q='.urlencode($opl).'&qft=+filterui:imagesize-large&safe=off';
     $data = curl_init();
